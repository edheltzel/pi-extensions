/**
 * Better Ask User Tool Extension - Interactive question UI for pi-coding-agent
 *
...
const BETTER_ASK_USER_VERSION: string = (_require("../package.json") as { version: string })
  .version;
...
type BetterAskUserOptionInput = QuestionOption | string;

type BetterAskUserDisplayMode = "overlay" | "inline";

interface BetterAskUserParams {
  question: string;
  context?: string;
  options?: BetterAskUserOptionInput[];
  allowMultiple?: boolean;
  allowFreeform?: boolean;
  allowComment?: boolean;
  displayMode?: BetterAskUserDisplayMode;
  overlayToggleKey?: string | null;
  commentToggleKey?: string | null;
  timeout?: number;
}

const BetterAskUserParameters = Type.Object({
  ...
  allowComment: Type.Optional(
    Type.Boolean({
      description:
        "Collect an optional comment after selecting one or more options. Default: PI_BETTER_ASK_USER_ALLOW_COMMENT env var if set, otherwise false.",
    }),
  ),
  displayMode: Type.Optional(
    StringEnum(["overlay", "inline"] as const, {
      description:
        "UI rendering mode. 'overlay' shows a centered modal, 'inline' renders in-place. Default: PI_BETTER_ASK_USER_DISPLAY_MODE env var if set, otherwise 'overlay'. Omit to respect the user's configured preference.",
    }),
  ),
  overlayToggleKey: Type.Optional(
    Type.String({
      description:
        "Shortcut for hiding/showing the overlay popup (overlay mode only), e.g. 'alt+o' or 'ctrl+shift+h'. Pass 'off' to disable. Default: PI_BETTER_ASK_USER_OVERLAY_TOGGLE_KEY env var if set, otherwise 'alt+o'.",
    }),
  ),
  commentToggleKey: Type.Optional(
    Type.String({
      description:
        "Shortcut for toggling the optional comment/extra-context row when allowComment is true, e.g. 'ctrl+g'. Pass 'off' to disable. Default: PI_BETTER_ASK_USER_COMMENT_TOGGLE_KEY env var if set, otherwise 'ctrl+g'.",
    }),
  ),
...
type BetterAskUserPreviewQuestion = Static<typeof BetterAskUserParameters>;
...
type BetterAskUserUIResult = BetterAskUserResponse;

type BetterAskUserExecutionDetails = BetterAskUserToolDetails | { error: string };

function parseBetterAskUserPreviewQuestions(value: unknown): BetterAskUserPreviewQuestion[] {
  if (!Array.isArray(value) || value.length === 0) {
    throw new Error("Preview fixture must be a non-empty JSON array of questions");
  }

  return value.map((entry, index) => {
    const number = index + 1;
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw new Error(`Question ${number} must be a JSON object`);
    }

    const question = entry as Record<string, unknown>;
    if (typeof question.question !== "string" || !question.question.trim()) {
      throw new Error(`Question ${number} must include a non-empty question`);
    }
    if (question.context !== undefined && typeof question.context !== "string") {
      throw new Error(`Question ${number} context must be a string`);
    }
    if (question.options !== undefined) {
      if (!Array.isArray(question.options)) {
        throw new Error(`Question ${number} options must be an array`);
      }
      for (const [optionIndex, option] of question.options.entries()) {
        if (!option || typeof option !== "object" || Array.isArray(option)) {
          throw new Error(`Question ${number} option ${optionIndex + 1} must be an object`);
        }
        const candidate = option as Record<string, unknown>;
        if (typeof candidate.title !== "string" || !candidate.title.trim()) {
          throw new Error(
            `Question ${number} option ${optionIndex + 1} must include a non-empty title`,
          );
        }
        if (candidate.description !== undefined && typeof candidate.description !== "string") {
          throw new Error(
            `Question ${number} option ${optionIndex + 1} description must be a string`,
          );
        }
      }
    }
    ...
    for (const key of ["allowMultiple", "allowFreeform", "allowComment"] as const) {
      if (question[key] !== undefined && typeof question[key] !== "boolean") {
        throw new Error(`Question ${number} ${key} must be a boolean`);
      }
    }
    if (
      question.displayMode !== undefined &&
      question.displayMode !== "overlay" &&
      question.displayMode !== "inline"
    ) {
      throw new Error(`Question ${number} displayMode must be overlay or inline`);
    }
    for (const key of ["overlayToggleKey", "commentToggleKey"] as const) {
      if (question[key] !== undefined && typeof question[key] !== "string") {
        throw new Error(`Question ${number} ${key} must be a string`);
      }
    }
    if (question.timeout !== undefined && typeof question.timeout !== "number") {
      throw new Error(`Question ${number} timeout must be a number`);
    }

    return question as BetterAskUserPreviewQuestion;
  });
}
...
function createFreeformResponse(text: string | null | undefined): BetterAskUserResponse | null {
  const trimmed = text?.trim();
...
function formatResponseSummary(response: BetterAskUserResponse): string {
...
function isSelectionResponse(
  response: BetterAskUserResponse,
): response is Extract<BetterAskUserResponse, { kind: "selection" }> {
...
interface ResolvedBetterAskUserShortcuts {
...
type BetterAskUserMode = "select" | "freeform" | "comment";
...
class MultiSelectList implements Component {
  ...
  render(width: number): string[] {
    ...
    const startIndex = Math.max(
      0,
      Math.min(this.selectedIndex - Math.floor(maxVisible / 2), count - maxVisible),
    );
    ...
    if (startIndex > 0 || endIndex < count) {
      lines.push(
        theme.fg("dim", truncateToWidth(`  (${this.selectedIndex + 1}/${count})`, width, "")),
      );
    }
...
class WrappedSingleSelectList implements Component {
  ...
  private getSplitPaneWidths(width: number): { left: number; right: number } | null {
    ...
    if (
      availableWidth <
      SINGLE_SELECT_SPLIT_PANE_LEFT_MIN_WIDTH + SINGLE_SELECT_SPLIT_PANE_RIGHT_MIN_WIDTH
    ) {
      return null;
    }
 ...
 */
class BetterAskUserComponent extends Container {
  ...
  private allowFreeform: boolean;
  private allowComment: boolean;
  private displayMode: BetterAskUserDisplayMode;
  private tui: TUI;
  private theme: Theme;
  private keybindings: KeybindingsManager;
  private shortcuts: ResolvedBetterAskUserShortcuts;
  private onDone: (result: BetterAskUserUIResult | null) => void;

  private mode: BetterAskUserMode = "select";
  ...
  constructor(
    ...
    theme: Theme,
    keybindings: KeybindingsManager,
    shortcuts: ResolvedBetterAskUserShortcuts,
    onDone: (result: BetterAskUserUIResult | null) => void,
  ) {
    super();
  ...
  private buildEditorModeHeaderLines(width: number): string[] {
    if (this.mode === "comment") {
      const selectedLabel =
        this.pendingSelections.length === 1 ? "Selected option:" : "Selected options:";
      return [
  ...
  private renderPromptPane(promptLines: string[], budget: number, width: number): string[] {
    ...
    this.promptScrollOffset = Math.max(
      0,
      Math.min(this.promptScrollOffset, this.promptMaxScrollOffset),
    );

    const visibleLines = promptLines.slice(
      this.promptScrollOffset,
      this.promptScrollOffset + viewportRows,
    );
  ...
  private updateHelpText(): void {
    ...
    const promptScrollHint =
      this.displayMode === "overlay" ? literalHint(theme, "PgUp/PgDn", "prompt") : null;
    const commentHint =
  ...
  private showFreeformMode(): void {
    ...
    this.modeContainer.addChild(
      new Text(this.theme.fg("accent", this.theme.bold("Custom response")), 1, 0),
    );
 ...
 */
async function betterAskUserViaDialogs(
  ...
  allowComment: boolean,
  timeout?: number,
): Promise<BetterAskUserUIResult | null> {
  ...
  if (selected === FREEFORM_SENTINEL) {
    const answer = (await ui.input(prompt, "Type your answer...", dialogOpts)) as
      | string
      | undefined;
...
export default function (pi: ExtensionAPI) {
  const betterAskUserTool = {
    ...
    promptGuidelines: [
      "Before calling better_ask_user, gather context with tools (read/web/ref) and pass a short summary via the context field.",
      "Use better_ask_user when the user's intent is ambiguous, when a decision requires explicit user input, or when multiple valid options exist.",
      "Ask exactly one focused question per better_ask_user call.",
      "Do not combine multiple numbered, multipart, or unrelated questions into one better_ask_user prompt.",
    ],
    // Block other tool calls in the same assistant turn until the user answers,
    // so the model can't batch better_ask_user with bash/edit/write and let those run
    // (potentially with side effects) before the user sees the prompt.
    executionMode: "sequential",
    parameters: BetterAskUserParameters,
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const {
        ...
        commentToggleKey,
        timeout,
      } = params as BetterAskUserParams;
      const envMode = process.env.PI_BETTER_ASK_USER_DISPLAY_MODE?.trim().toLowerCase();
      const envDisplayMode: BetterAskUserDisplayMode | undefined =
        envMode === "overlay" || envMode === "inline" ? envMode : undefined;
      const effectiveDisplayMode: BetterAskUserDisplayMode =
        displayMode ?? envDisplayMode ?? "overlay";
      const allowComment =
        requestedAllowComment ??
        parseBooleanPreference(process.env.PI_BETTER_ASK_USER_ALLOW_COMMENT) ??
        false;
      const shortcuts: ResolvedBetterAskUserShortcuts = {
        overlayToggle: resolveShortcut(
          overlayToggleKey,
          process.env.PI_BETTER_ASK_USER_OVERLAY_TOGGLE_KEY,
          DEFAULT_OVERLAY_TOGGLE_KEY,
        ),
        commentToggle: resolveShortcut(
          commentToggleKey,
          process.env.PI_BETTER_ASK_USER_COMMENT_TOGGLE_KEY,
          ...
          content: [
            {
              ...
              text:
                ...
                `Call better_ask_user again with corrected options.`,
            },
          ],
          isError: true,
      ...
      if (!ctx.hasUI || !ctx.ui) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Better Ask User requires interactive mode. Please answer:\n\n${question}${contextText}${optionText}${freeformHint}${commentHint}`,
            },
          ],
          isError: true,
          details: {
      ...
      if (options.length === 0) {
        ...
        pi.events.emit("better-ask:answered", { question, context: normalizedContext, response });
        return {
      ...
      let result: BetterAskUserUIResult | null;
      ...
      try {
        const customFactory = (
          ...
          theme: Theme,
          keybindings: KeybindingsManager,
          done: (result: BetterAskUserUIResult | null) => void,
        ) => {
          if (signal) {
          ...
          return new BetterAskUserComponent(
            question,
            normalizedContext,
            options,
            allowMultiple,
        ...
        const customResult = await ctx.ui.custom<BetterAskUserUIResult | null>(
          customFactory,
        ...
        if (customResult !== undefined) {
          ...
          result = await betterAskUserViaDialogs(
            ctx.ui,
            question,
            normalizedContext,
            options,
        ...
        return {
          content: [{ type: "text", text: `Better Ask User tool failed: ${message}` }],
          isError: true,
      ...
      if (result === null) {
        pi.events.emit("better-ask:cancelled", { question, context: normalizedContext, options });
        return {
          content: [{ type: "text", text: "User cancelled the question" }],
          details: {
            question,
      ...
      pi.events.emit("better-ask:answered", {
        question,
        context: normalizedContext,
        response: result,
      });
    ...
    renderCall(args, theme) {
      ...
      let text = theme.fg("toolTitle", theme.bold("better_ask_user "));
    ...
    renderResult(result, options, theme) {
      const details = result.details as (BetterAskUserToolDetails & { error?: string }) | undefined;
  ...
  } satisfies ToolDefinition<typeof BetterAskUserParameters, BetterAskUserExecutionDetails>;

  pi.registerTool(betterAskUserTool);
  pi.registerCommand("better-ask-preview", {
    description: "Run better_ask_user questions from a JSON fixture using the real Pi UI",
    handler: async (args, ctx) => {
      if (ctx.mode !== "tui") {
        ctx.ui.notify("/better-ask-preview requires interactive TUI mode", "error");
        return;
      }

      const fixtureArgument = args.trim();
      if (!fixtureArgument) {
        ctx.ui.notify("Usage: /better-ask-preview path/to/questions.json", "warning");
        return;
      }

      let questions: BetterAskUserPreviewQuestion[];
      try {
        const fixturePath = resolve(ctx.cwd, fixtureArgument);
        const fixture = JSON.parse(await readFile(fixturePath, "utf8")) as unknown;
        questions = parseBetterAskUserPreviewQuestions(fixture);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        ctx.ui.notify(`Better Ask User preview failed: ${message}`, "error");
        return;
      }

      for (const [index, question] of questions.entries()) {
        const result = await betterAskUserTool.execute(
          `better-ask-preview-${index + 1}`,
          question,
          undefined,
          undefined,
          ctx,
        ...
        if ("error" in details) {
          ctx.ui.notify(
            `Better Ask User preview failed at question ${index + 1}: ${details.error}`,
            "error",
          );
          return;
        }
        if (details.cancelled) {
          ctx.ui.notify(
            `Better Ask User preview stopped at question ${index + 1} of ${questions.length}.`,
            "warning",
          );
          return;
        }
      ...
      ctx.ui.notify(
        `Better Ask User preview complete: ${questions.length} questions answered.`,
        "info",
      );
    },
  });
