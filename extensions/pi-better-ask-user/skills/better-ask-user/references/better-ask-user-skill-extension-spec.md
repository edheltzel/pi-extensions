# Better Ask User Skill × Extension Interaction Spec

## Purpose

This document defines a minimal decision-gating protocol for using the `better-ask-user` skill with the `better_ask_user` tool.

Goal: require explicit user decisions at high-impact or ambiguous boundaries before implementation continues.

---

## 1) Trigger Matrix (When to Call `better_ask_user`)
...
4. **Ask one focused question**
   - call `better_ask_user` for one decision at a time
5. **Commit and proceed**
   - restate chosen option and implement accordingly

### Retry/cancel policy

- Max **2** `better_ask_user` attempts for the same decision boundary.
...
The `better_ask_user` tool accepts an optional `displayMode` parameter:
...
- Omit `displayMode` to respect the user's configured preference (`PI_BETTER_ASK_USER_DISPLAY_MODE` environment variable).
