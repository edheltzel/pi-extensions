...
- **Breaking:** Renamed the package from `@edheltzel/pi-ask-user` to `@edheltzel/pi-better-ask-user` and moved it to `packages/pi-better-ask-user`.
- **Breaking:** Renamed the registered tool from `ask_user` to `better_ask_user`, the bundled skill from `ask-user` to `better-ask-user`, and the environment prefix from `PI_ASK_USER_*` to `PI_BETTER_ASK_USER_*`.
- **Breaking:** Renamed emitted answer lifecycle events from `ask:*` to `better-ask:*` and the Herdr blocked-state label to `better_ask_user`.

## 0.13.0-herdr.1 - 2026-07-18
...
- The previous Atlas-maintained fork identified its repository as `edheltzel/pi-ask-user` while retaining upstream MIT licensing and authorship.
