...
type BetterAskUserComponentFactory = (
  tui: unknown,
  theme: unknown,
  keybindings: unknown,
...
function enableHerdr(): void {
  ...
  process.env.HERDR_SOCKET_PATH = `/tmp/pi-better-ask-user-test-${crypto.randomUUID()}.sock`;
...
async function setupExtension(): Promise<{
  ...
  const { default: betterAskUserExtension } = await import("../src/index");
...
describe("better_ask_user", () => {
  ...
  describe.sequential("Herdr lifecycle", () => {
  ...
  test("registers the breaking better_ask_user name with sequential execution", async () => {
    const tool = await setupTool();
    expect(tool.name).toBe("better_ask_user");
    expect(tool.executionMode).toBe("sequential");
  });

  describe("preview command", () => {
    async function createFixture(contents: unknown): Promise<string> {
      const directory = await mkdtemp(join(tmpdir(), "pi-better-ask-user-preview-"));
    ...
    test("runs every question in a JSON fixture without an agent turn", async () => {
      const fixturePath = await createFixture([
        { question: "What is your first answer?" },
        {
          question: "What is your second answer?",
          context: "This is the second preview question.",
        },
      ]);
      const { commands } = await setupExtension();
      const command = commands.get("better-ask-preview");
      const prompts: string[] = [];
      const notifications: Array<{ message: string; level: string }> = [];
      const answers = ["First", "Second"];

      expect(command).toBeDefined();
      await command!.handler(fixturePath, {
        mode: "tui",
        hasUI: true,
        cwd: process.cwd(),
        ui: {
      ...
      expect(prompts).toEqual([
        ...
        "What is your second answer?\n\nContext:\nThis is the second preview question.",
      ]);
      expect(notifications).toContainEqual({
        message: "Better Ask User preview complete: 2 questions answered.",
        level: "info",
      });
    });

    test("stops the fixture after a cancelled question", async () => {
      const fixturePath = await createFixture([
      ...
      expect(notifications.at(-1)).toEqual({
        message: "Better Ask User preview stopped at question 1 of 2.",
        level: "warning",
      });
    });

    test("rejects malformed fixture questions before opening the UI", async () => {
      const fixturePath = await createFixture([{ context: "Missing question" }]);
      const { commands } = await setupExtension();
      const command = commands.get("better-ask-preview");
      const notifications: Array<{ message: string; level: string }> = [];
      ...
      expect(notifications.at(-1)?.message).toContain(
        "Question 1 must include a non-empty question",
      );
    });
  });
  ...
  test("uses PI_BETTER_ASK_USER_DISPLAY_MODE env var when call-level displayMode is omitted", async () => {
    stubEnv("PI_BETTER_ASK_USER_DISPLAY_MODE", "inline");
    const tool = await setupTool();
    let capturedOptions: any;
  ...
  test("normalizes PI_BETTER_ASK_USER_DISPLAY_MODE before applying it", async () => {
    stubEnv("PI_BETTER_ASK_USER_DISPLAY_MODE", " INLINE ");
    const tool = await setupTool();
  ...
  test("call-level displayMode overrides PI_BETTER_ASK_USER_DISPLAY_MODE env var", async () => {
    stubEnv("PI_BETTER_ASK_USER_DISPLAY_MODE", "inline");
    const tool = await setupTool();
    let capturedOptions: any;
  ...
  test("ignores unrecognised PI_BETTER_ASK_USER_DISPLAY_MODE value and falls back to overlay", async () => {
    stubEnv("PI_BETTER_ASK_USER_DISPLAY_MODE", "fullscreen");
    const tool = await setupTool();
    let capturedOptions: any;
  ...
  describe("overlay hide/show toggle (alt+o)", () => {
    ...
    test("PI_BETTER_ASK_USER_OVERLAY_TOGGLE_KEY env var overrides default", async () => {
      stubEnv("PI_BETTER_ASK_USER_OVERLAY_TOGGLE_KEY", "alt+h");
      const tool = await setupTool();
      const { handle, calls } = createOverlayHandle();
      let inputHandler: ((data: string) => any) | undefined;
  ...
  test("uses PI_BETTER_ASK_USER_ALLOW_COMMENT when allowComment is omitted", async () => {
    stubEnv("PI_BETTER_ASK_USER_ALLOW_COMMENT", "true");
    const tool = await setupTool();
    ...
    const result = await tool.execute(
      ...
      undefined,
      {
        hasUI: true,
        ui: {
          custom: async (factory: BetterAskUserComponentFactory) => {
  ...
  test("call-level allowComment false overrides PI_BETTER_ASK_USER_ALLOW_COMMENT", async () => {
    stubEnv("PI_BETTER_ASK_USER_ALLOW_COMMENT", "true");
    const tool = await setupTool();
  ...
  test("keeps freeform available when search filters out every option", async () => {
    ...
    const answeredEvent = emittedEvents.find((event) => event.name === "better-ask:answered");
  ...
  describe("issue #22 option normalization", () => {
    test("salvages common option title aliases when schema proxies mangle the shape", async () => {
      ...
      const result = await tool.execute(
        ...
        {
          question: "Pick one",
          options: [{ label: "A" }, { text: "B" }, { value: "C" }, { name: "D" }, { option: "E" }],
          allowFreeform: false,
        },
        undefined,
        undefined,
      ...
      expect(result.details.options.map((option: { title: string }) => option.title)).toEqual([
        "A",
        "B",
        "C",
        "D",
